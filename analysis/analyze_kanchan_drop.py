"""Analyze wait quality after a kanchan-drop-looking river pattern."""
...
CODE_PATHS = (
    "analysis/analyze_kanchan_drop.py",
    "src/mahjong_analysis/kanchan_drop_analysis.py",
    ...
    "docs/specs/kanchan-drop-analysis.md",
...
def main(argv: list[str] | None = None) -> int:
    ...
    def report(result) -> None:
        ...
        inner_count = result.overall.by_order["inner_first"].record_count
        outer_count = result.overall.by_order["outer_first"].record_count
        print(
            f"year={result.year} records={result.overall.record_count:,} "
            f"pattern={pattern_count:,} inner={inner_count:,} outer={outer_count:,} "
            f"elapsed={time.monotonic() - started:.1f}s",
            file=sys.stderr,
            flush=True,
        )

    results = run_kanchan_drop_tasks(
        tuple(
            AnnualKanchanDropTask(
                entry["year"],
    ...
    document = kanchan_drop_results_document(results)
    ...
    document["metadata"] = {
        "analysis_name": "kanchan-drop-analysis-v1",
        "schema_version": 1,
        ...
        "definitions": {
            "pattern": "adjacent own suited discards in the same suit with normalized rank difference 2 and second tsumogiri=false",
            "A": "tedashi then tedashi",
            "B": "tsumogiri then tedashi; equal status to A in the main pattern",
            "classification": "exclusive none/a_only/b_only/both using all matching candidates",
            "representative_candidate": "last completed matching candidate",
            "distance": "riichi_discard_number minus representative second discard_number",
            "central_distance": "abs(rank - 5)",
            "inner_first": "representative first rank is closer to 5 than the second rank",
            "outer_first": "representative second rank is closer to 5 than the first rank",
            "symmetric": "representative ranks are equally distant from 5; shape 46",
        ...
        "validation": {
            ...
            "classification_and_representative_partitions": "PASS",
            "year_turn_distance_partitions": "PASS",
            "toitsu_status_partition": "PASS",
    ...
    print(
        json.dumps(
            document["recent_2020_2025"]["overall"], ensure_ascii=True, sort_keys=True
        )
    )
    return 0
