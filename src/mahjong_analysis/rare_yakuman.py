...
class RareYakumanResult:
    ...
    def __post_init__(self) -> None:
        ...
        for name, value in (
            ...
            _require_non_negative_int(value, name)
        if self.matching_kyokus > self.total_kyokus:
            raise ValueError("matching_kyokus cannot exceed total_kyokus")
    ...
    @property
    def probability_per_kyoku(self) -> float | None:
        """Return matching kyokus divided by all kyokus."""
        if self.total_kyokus == 0:
            return None
        return self.matching_kyokus / self.total_kyokus

    @property
    def one_in_n_kyokus(self) -> float | None:
        """Return the reciprocal kyoku frequency, or None when there are no hits."""
        if self.matching_kyokus == 0:
            return None
        return self.total_kyokus / self.matching_kyokus
    ...
    for event_index, event in enumerate(kyoku[1:-1], start=1):
        ...
        if event_type == "tsumo":
            ...
            if result_type == "ryukyoku":
                raise ValueError(f"event {event_index}: hora follows ryukyoku")
            result_type = "hora"
            ...
            if result_type is not None:
                raise ValueError(f"event {event_index}: duplicate terminal result")
            ...
            raise ValueError(f"event {event_index}: unexpected {event_type}")
        else:
            raise ValueError(
    ...
    *,
    ...
    for kyoku_index, kyoku_events in enumerate(kyokus):
        try:
            ...
            matching = tuple(
                hora
                for hora in replay_kyoku_horas(kyoku_events)
                if detector(hora.owned_tiles)
            )
...
def scan_rare_yakuman(
    ...
    *,
    ...
    tasks = tuple((str(root), year, yaku) for year in sorted(selected_years))
    if workers == 1:
...
def result_to_dict(result: RareYakumanResult) -> dict[str, Any]:
    ...
    return {
        ...
        "probability_per_kyoku": result.probability_per_kyoku,
        "one_in_n_kyokus": result.one_in_n_kyokus,
        "hits": [
            {
...
def _scan_year(task: tuple[str, int, str]) -> RareYakumanResult:
    raw_root_text, year, yaku = task
    ...
    hits: list[RareYakumanHit] = []
    for path in paths:
        result = analyze_rare_yakuman_game(path, raw_root=raw_root, yaku=yaku)
        total_kyokus += result.total_kyokus
...
def _initial_states(kyoku: Sequence[dict[str, Any]]) -> list[_PlayerState]:
    if not isinstance(kyoku, Sequence) or isinstance(kyoku, (str, bytes)):
        raise TypeError("kyoku must be a sequence of events")
    if len(kyoku) < 3:
        raise ValueError("kyoku must contain start, result, and end events")
    ...
    for actor, tehai in enumerate(tehais):
        if not isinstance(tehai, list) or len(tehai) != 13:
            raise ValueError(f"event 0, actor {actor}: initial hand must have 13 tiles")
...
def _event_type(event: dict[str, Any], event_index: int) -> str:
    if not isinstance(event, dict):
        raise TypeError(f"event {event_index}: event must be a dict")
    value = event.get("type")
    if not isinstance(value, str) or not value:
        raise ValueError(f"event {event_index}: type must be a non-empty string")
    return value


def _event_error(event_index: int, actor: int, message: str) -> ValueError:
