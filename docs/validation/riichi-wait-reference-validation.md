...
注意（2026-09-12追記）: 以下の正式比較は5枚目待ちに関する共有仕様バグの修正前に
実施した記録である。独立アルゴリズム同士の一致でも、共有する仕様の誤りは検出でき
なかった。修正後の全範囲再比較済みという意味ではない。影響診断は末尾に記録する。
...
旧仕様で出力済みの年度は修正後canonical datasetへ流用しない。failed output rootは
そのまま保持し、修正・検証・コミット後にcleanな別rootから2009〜2025を再生成する。
この記録は旧full exportをresumeしてよいという判断ではない。
