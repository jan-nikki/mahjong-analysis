...
扱う。既存の `is_dealer_double_riichi()` と同じ成立条件を共有し、
親の第一打リーチに限らない全actor向けの判定を別実装として
重複させない。
...
本番実装とは独立したreference経路を設ける。

- 本番の待ち判定モジュールをimportしない
- MJAIを独自にストリーミング走査する
- 成立リーチ候補を `filename + reach_line` で比較する
- 完成形を先に列挙する本番方式とは異なる方法で、未完成部分から
  `wait_tile × hand_type × wait_shape` を直接列挙する
- 件数だけでなく、個々のリーチの `wait_tiles` と `wait_details` を
  完全一致で比較する
- reference側が本番側の正規化関数や定数を共有していないことを
  テストで確認する

補助検証として、版を固定した独立ライブラリで34牌種を1枚ずつ加えた
和了判定を行い、`wait_tiles` を比較してよい。ただし、そのライブラリを
本番分析の依存にはしない。待ち形については、本仕様の人工牌姿を
独立した既知ケースとして扱う。

実牌譜検証では、カテゴリ別にpositive/negativeケースを決定論的に抽出し、
`filename` と各イベント行番号からMJAI原本を目視できるようにする。
後に同じactorが和了した場合は、`hora.pai` の正規化牌種がリーチ時の
`wait_tiles` に含まれることも検査する。

性能については、年次全牌譜走査へ進む前にレコード生成を計測する。
必要な場合は、検証の意味を変えずに待ち判定の実行を1回へ削減する。
